...
namespace Sapphire
{
	...
	public:
		...
		Color Abs() const { return Color(Sapphire::Abs(r_), Sapphire::Abs(g_), Sapphire::Abs(b_), Sapphire::Abs(a_)); }

		/// Test for equality with another color with epsilon.
		bool Equals(const Color& rhs) const
		{
			return Sapphire::Equals(r_, rhs.r_) && Sapphire::Equals(g_, rhs.g_) && Sapphire::Equals(b_, rhs.b_) && Sapphire::Equals(a_, rhs.a_);
		}
