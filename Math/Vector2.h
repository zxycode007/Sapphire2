...
namespace Sapphire
{
	...
	public:
		...
		float AbsDotProduct(const Vector2& rhs) const { return Sapphire::Abs(x_ * rhs.x_) + Sapphire::Abs(y_ * rhs.y_); }

		/// Return absolute vector.
		Vector2 Abs() const { return Vector2(Sapphire::Abs(x_), Sapphire::Abs(y_)); }

		/// Linear interpolation with another vector.
		Vector2 Lerp(const Vector2& rhs, float t) const { return *this * (1.0f - t) + rhs * t; }

		/// Test for equality with another vector with epsilon.
		bool Equals(const Vector2& rhs) const { return Sapphire::Equals(x_, rhs.x_) && Sapphire::Equals(y_, rhs.y_); }

		/// Return whether is NaN.
		bool IsNaN() const { return Sapphire::IsNaN(x_) || Sapphire::IsNaN(y_); }
