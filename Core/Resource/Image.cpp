		...
		if (fileID == "DDS ")
		{
			...
			if (hasDXGI)
			{
				...
				case DDS_DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
					...
					break;
				default:
					SAPPHIRE_LOGERROR("Unrecognized DDS DXGI image format");
					return false;
				}
			...
			case 0:
				...
				{
					SAPPHIRE_LOGERROR("Unsupported DDS pixel byte size");
					return false;
				}
			...
			default:
				SAPPHIRE_LOGERROR("Unrecognized DDS image format");
				return false;
			}
			...
			if (compressedFormat_ == CF_RGBA)
			{
				SAPPHIRE_PROFILE(ConvertDDSToRGBA);
...
#define ADJUSTSHIFT(mask, l, r) \
		...
		else if (fileID == "\253KTX")
		{
			...
			{
				SAPPHIRE_LOGERROR("Big-endian KTX files not supported");
				return false;
			}

			if (type != 0 || format != 0)
			{
				SAPPHIRE_LOGERROR("Uncompressed KTX files not supported");
				return false;
			}

			if (faces > 1 || depth > 1)
			{
				SAPPHIRE_LOGERROR("3D or cube KTX files not supported");
				return false;
			}

			if (mipmaps == 0)
			{
				SAPPHIRE_LOGERROR("KTX files without explicitly specified mipmap count not supported");
				return false;
			}
			...
			{
				SAPPHIRE_LOGERROR("Unsupported texture format in KTX file");
				return false;
			}
			...
			for (unsigned i = 0; i < mipmaps; ++i)
			{
				...
				{
					SAPPHIRE_LOGERROR("KTX mipmap level data size exceeds file size");
					return false;
				}
		...
		else if (fileID == "PVR\3")
		{
			...
			{
				SAPPHIRE_LOGERROR("3D or cube PVR files not supported");
				return false;
			}

			if (mipmapCount == 0)
			{
				SAPPHIRE_LOGERROR("PVR files without explicitly specified mipmap count not supported");
				return false;
			}
			...
			if (compressedFormat_ == CF_NONE)
			{
				SAPPHIRE_LOGERROR("Unsupported texture format in PVR file");
				return false;
			}
		...
		else
		{
			...
			if (!pixelData)
			{
				SAPPHIRE_LOGERROR("Could not load image " + source.GetName() + ": " + String(stbi_failure_reason()));
				return false;
			}
	...
	{
		SAPPHIRE_PROFILE(SaveImage);

		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not save compressed image " + GetName());
			return false;
		}

		if (!data_)
		{
			SAPPHIRE_LOGERROR("Can not save zero-sized image " + GetName());
			return false;
		}
	...
	bool Image::SetSize(int width, int height, int depth, unsigned components)
	{
		...
		if (components > 4)
		{
			SAPPHIRE_LOGERROR("More than 4 color components are not supported");
			return false;
		}
	...
	void Image::SetData(const unsigned char* pixelData)
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not set new pixel data for a compressed image");
			return;
		}
	...
	bool Image::LoadColorLUT(Deserializer& source)
	{
		...
		{
			SAPPHIRE_LOGERROR("Invalid image format, can not load image");
			return false;
		}
	...
	bool Image::FlipHorizontal()
	{
		...
		if (depth_ > 1)
		{
			SAPPHIRE_LOGERROR("FlipHorizontal not supported for 3D images");
			return false;
		}
		...
		else
		{
			...
			{
				SAPPHIRE_LOGERROR("FlipHorizontal not yet implemented for other compressed formats than RGBA & DXT1,3,5");
				return false;
			}
			...
			for (unsigned i = 0; i < numCompressedLevels_; ++i)
			{
				...
				if (!level.data_)
				{
					SAPPHIRE_LOGERROR("Got compressed level with no data, aborting horizontal flip");
					return false;
				}
	...
	bool Image::FlipVertical()
	{
		...
		if (depth_ > 1)
		{
			SAPPHIRE_LOGERROR("FlipVertical not supported for 3D images");
			return false;
		}
		...
		else
		{
			if (compressedFormat_ > CF_DXT5)
			{
				SAPPHIRE_LOGERROR("FlipVertical not yet implemented for other compressed formats than DXT1,3,5");
				return false;
			}
			...
			for (unsigned i = 0; i < numCompressedLevels_; ++i)
			{
				...
				if (!level.data_)
				{
					SAPPHIRE_LOGERROR("Got compressed level with no data, aborting vertical flip");
					return false;
				}
	...
	{
		SAPPHIRE_PROFILE(ResizeImage);

		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Resize not supported for compressed images");
			return false;
		}

		if (depth_ > 1)
		{
			SAPPHIRE_LOGERROR("Resize not supported for 3D images");
			return false;
		}
	...
	void Image::ClearInt(unsigned uintColor)
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Clear not supported for compressed images");
			return;
		}
	...
	bool Image::SaveBMP(const String& fileName) const
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not save compressed image to BMP");
			return false;
		}
	...
	bool Image::SavePNG(const String& fileName) const
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not save compressed image to PNG");
			return false;
		}
	...
	bool Image::SaveTGA(const String& fileName) const
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not save compressed image to TGA");
			return false;
		}
	...
	bool Image::SaveJPG(const String& fileName, int quality) const
	{
		...
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not save compressed image to JPG");
			return false;
		}
	...
	SharedPtr<Image> Image::GetNextLevel() const
	{
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not generate mip level from compressed data");
			return SharedPtr<Image>();
		}
		if (components_ < 1 || components_ > 4)
		{
			SAPPHIRE_LOGERROR("Illegal number of image components for mip level generation");
			return SharedPtr<Image>();
		}
		...
		SAPPHIRE_PROFILE(CalculateImageMipLevel);
	...
	SharedPtr<Image> Image::ConvertToRGBA() const
	{
		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not convert compressed image to RGBA");
			return SharedPtr<Image>();
		}
		if (components_ < 1 || components_ > 4)
		{
			SAPPHIRE_LOGERROR("Illegal number of image components for conversion to RGBA");
			return SharedPtr<Image>();
		}
		if (!data_)
		{
			SAPPHIRE_LOGERROR("Can not convert image without data to RGBA");
			return SharedPtr<Image>();
		}
	...
	CompressedLevel Image::GetCompressedLevel(unsigned index) const
	{
		...
		if (compressedFormat_ == CF_NONE)
		{
			SAPPHIRE_LOGERROR("Image is not compressed");
			return level;
		}
		if (index >= numCompressedLevels_)
		{
			SAPPHIRE_LOGERROR("Compressed image mip level out of bounds");
			return level;
		}
		...
		if (compressedFormat_ == CF_RGBA)
		{
			...
			for (;;)
			{
				...
				{
					SAPPHIRE_LOGERROR("Compressed level is outside image data. Offset: " + String(offset) + " Size: " + String(level.dataSize_) +
	...
	Image* Image::GetSubimage(const IntRect& rect) const
	{
		...
		if (depth_ > 1)
		{
			SAPPHIRE_LOGERROR("Subimage not supported for 3D images");
			return 0;
		}

		if (rect.left_ < 0 || rect.top_ < 0 || rect.right_ > width_ || rect.bottom_ > height_ || !rect.Width() || !rect.Height())
		{
			SAPPHIRE_LOGERROR("Can not get subimage from image " + GetName() + " with invalid region");
			return 0;
		}
		...
		else
		{
			...
			if (!subimageLevels)
			{
				SAPPHIRE_LOGERROR("Subimage region from compressed image " + GetName() + " did not produce any data");
				return 0;
			}
	...
	SDL_Surface* Image::GetSDLSurface(const IntRect& rect) const
	{
		...
		if (depth_ > 1)
		{
			SAPPHIRE_LOGERROR("Can not get SDL surface from 3D image");
			return 0;
		}

		if (IsCompressed())
		{
			SAPPHIRE_LOGERROR("Can not get SDL surface from compressed image " + GetName());
			return 0;
		}

		if (components_ < 3)
		{
			SAPPHIRE_LOGERROR("Can not get SDL surface from image " + GetName() + " with less than 3 components");
			return 0;
		}
		...
		if (surface)
		{
			...
			SAPPHIRE_LOGERROR("Failed to create SDL surface from image " + GetName());
	...
	void Image::PrecalculateLevels()
	{
		...
		SAPPHIRE_PROFILE(PrecalculateImageMipLevels);
