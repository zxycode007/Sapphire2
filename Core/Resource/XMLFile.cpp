...
namespace Sapphire
{
	...
	bool XMLFile::BeginLoad(Deserializer& source)
	{
		...
		if (!inherit.Empty())
		{
			...
			if (!inheritedXMLFile)
			{
				SAPPHIRE_LOGERRORF("Could not find inherited XML file: %s", inherit.CString());
				return false;
			}
	...
	void XMLFile::Patch(XMLElement patchElement)
	{
		...
		for (pugi::xml_node::iterator patch = root.begin(); patch != root.end(); patch++)
		{
			...
			if (!original)
			{
				SAPPHIRE_LOGERRORF("XML Patch failed with bad select: %s.", sel.value());
				continue;
			}
	...
	void XMLFile::PatchAdd(const pugi::xml_node& patch, pugi::xpath_node& original) const
	{
		...
		{
			SAPPHIRE_LOGERRORF("XML Patch failed calling Add due to not selecting a node, %s attribute was selected.",
	...
	void XMLFile::AddAttribute(const pugi::xml_node& patch, const pugi::xpath_node& original) const
	{
		...
		{
			SAPPHIRE_LOGERRORF("XML Patch failed calling Add due to attempting to add non text to an attribute for %s.", attribute.value());
			return;
		}
	...
	bool XMLFile::CombineText(const pugi::xml_node& patch, const pugi::xml_node& original, bool prepend) const
	{
		...
		{
			if (prepend)
				const_cast<pugi::xml_node&>(original).set_value(Sapphire::ToString("%s%s", patch.value(), original.value()).CString());
			else
				const_cast<pugi::xml_node&>(original).set_value(Sapphire::ToString("%s%s", original.value(), patch.value()).CString());
