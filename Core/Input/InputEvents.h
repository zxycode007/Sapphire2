...
namespace Sapphire
{
	...
	SAPPHIRE_EVENT(E_MOUSEBUTTONDOWN, MouseButtonDown)
	{
		SAPPHIRE_PARAM(P_BUTTON, Button);                // int
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
	}

	/// Mouse button released.
	SAPPHIRE_EVENT(E_MOUSEBUTTONUP, MouseButtonUp)
	{
		SAPPHIRE_PARAM(P_BUTTON, Button);                // int
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
	...
	/// Mouse moved.
	SAPPHIRE_EVENT(E_MOUSEMOVE, MouseMove)
	{
		SAPPHIRE_PARAM(P_X, X);                          // int (only when mouse visible)
		SAPPHIRE_PARAM(P_Y, Y);                          // int (only when mouse visible)
		SAPPHIRE_PARAM(P_DX, DX);                        // int
		SAPPHIRE_PARAM(P_DY, DY);                        // int
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
	}

	/// Mouse wheel moved.
	SAPPHIRE_EVENT(E_MOUSEWHEEL, MouseWheel)
	{
		SAPPHIRE_PARAM(P_WHEEL, Wheel);                  // int
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
	}

	/// Key pressed.
	SAPPHIRE_EVENT(E_KEYDOWN, KeyDown)
	{
		SAPPHIRE_PARAM(P_KEY, Key);                      // int
		SAPPHIRE_PARAM(P_SCANCODE, Scancode);            // int
		SAPPHIRE_PARAM(P_RAW, Raw);                      // uint
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
		SAPPHIRE_PARAM(P_REPEAT, Repeat);                // bool
	}

	/// Key released.
	SAPPHIRE_EVENT(E_KEYUP, KeyUp)
	{
		SAPPHIRE_PARAM(P_KEY, Key);                      // int
		SAPPHIRE_PARAM(P_SCANCODE, Scancode);            // int
		SAPPHIRE_PARAM(P_RAW, Raw);                      // uint
	...
	/// Text input event.
	SAPPHIRE_EVENT(E_TEXTINPUT, TextInput)
	{
		SAPPHIRE_PARAM(P_TEXT, Text);                    // String
		SAPPHIRE_PARAM(P_BUTTONS, Buttons);              // int
		SAPPHIRE_PARAM(P_QUALIFIERS, Qualifiers);        // int
	}

	/// Joystick connected.
	SAPPHIRE_EVENT(E_JOYSTICKCONNECTED, JoystickConnected)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
	}

	/// Joystick disconnected.
	SAPPHIRE_EVENT(E_JOYSTICKDISCONNECTED, JoystickDisconnected)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
	}

	/// Joystick button pressed.
	SAPPHIRE_EVENT(E_JOYSTICKBUTTONDOWN, JoystickButtonDown)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
		SAPPHIRE_PARAM(P_BUTTON, Button);                // int
	}
	...
	SAPPHIRE_EVENT(E_JOYSTICKBUTTONUP, JoystickButtonUp)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
		SAPPHIRE_PARAM(P_BUTTON, Button);                // int
	}
	...
	/// Joystick axis moved.
	SAPPHIRE_EVENT(E_JOYSTICKAXISMOVE, JoystickAxisMove)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
		SAPPHIRE_PARAM(P_AXIS, Button);                  // int
		SAPPHIRE_PARAM(P_POSITION, Position);            // float
	}

	/// Joystick POV hat moved.
	SAPPHIRE_EVENT(E_JOYSTICKHATMOVE, JoystickHatMove)
	{
		SAPPHIRE_PARAM(P_JOYSTICKID, JoystickID);        // int
		SAPPHIRE_PARAM(P_HAT, Button);                   // int
		SAPPHIRE_PARAM(P_POSITION, Position);            // int
	}

	/// Finger pressed on the screen.
	SAPPHIRE_EVENT(E_TOUCHBEGIN, TouchBegin)
	{
		SAPPHIRE_PARAM(P_TOUCHID, TouchID);              // int
		SAPPHIRE_PARAM(P_X, X);                          // int
		SAPPHIRE_PARAM(P_Y, Y);                          // int
		SAPPHIRE_PARAM(P_PRESSURE, Pressure);            // float
	}

	/// Finger released from the screen.
	SAPPHIRE_EVENT(E_TOUCHEND, TouchEnd)
	{
		SAPPHIRE_PARAM(P_TOUCHID, TouchID);              // int
		SAPPHIRE_PARAM(P_X, X);                          // int
		SAPPHIRE_PARAM(P_Y, Y);                          // int
	...
	SAPPHIRE_EVENT(E_TOUCHMOVE, TouchMove)
	{
		SAPPHIRE_PARAM(P_TOUCHID, TouchID);              // int
		SAPPHIRE_PARAM(P_X, X);                          // int
		SAPPHIRE_PARAM(P_Y, Y);                          // int
	...
	SAPPHIRE_EVENT(E_GESTURERECORDED, GestureRecorded)
	{
		SAPPHIRE_PARAM(P_GESTUREID, GestureID);          // unsigned
	}

	/// A recognized touch gesture was input by the user.
	SAPPHIRE_EVENT(E_GESTUREINPUT, GestureInput)
	{
		SAPPHIRE_PARAM(P_GESTUREID, GestureID);          // unsigned
		SAPPHIRE_PARAM(P_CENTERX, CenterX);              // int
		SAPPHIRE_PARAM(P_CENTERY, CenterY);              // int
		SAPPHIRE_PARAM(P_NUMFINGERS, NumFingers);        // int
		SAPPHIRE_PARAM(P_ERROR, Error);                  // float
	}

	/// Pinch/rotate multi-finger touch gesture motion update.
	SAPPHIRE_EVENT(E_MULTIGESTURE, MultiGesture)
	{
		SAPPHIRE_PARAM(P_CENTERX, CenterX);              // int
		SAPPHIRE_PARAM(P_CENTERY, CenterY);              // int
		SAPPHIRE_PARAM(P_NUMFINGERS, NumFingers);        // int
		SAPPHIRE_PARAM(P_DTHETA, DTheta);                // float (degrees)
		SAPPHIRE_PARAM(P_DDIST, DDist);                  // float
	}

	/// A file was drag-dropped into the application window.
	SAPPHIRE_EVENT(E_DROPFILE, DropFile)
	{
		SAPPHIRE_PARAM(P_FILENAME, FileName);            // String
	}

	/// Application input focus or minimization changed.
	SAPPHIRE_EVENT(E_INPUTFOCUS, InputFocus)
	{
		SAPPHIRE_PARAM(P_FOCUS, Focus);                  // bool
		SAPPHIRE_PARAM(P_MINIMIZED, Minimized);          // bool
	}

	/// OS mouse cursor visibility changed.
	SAPPHIRE_EVENT(E_MOUSEVISIBLECHANGED, MouseVisibleChanged)
	{
		SAPPHIRE_PARAM(P_VISIBLE, Visible);              // bool
	}

	/// Mouse mode changed.
	SAPPHIRE_EVENT(E_MOUSEMODECHANGED, MouseModeChanged)
	{
		SAPPHIRE_PARAM(P_MODE, Mode);                    // MouseMode
	}

	/// Application exit requested.
	SAPPHIRE_EVENT(E_EXITREQUESTED, ExitRequested)
	{
	}
