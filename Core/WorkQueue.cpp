...
namespace Sapphire
{
	...
	{
		SubscribeToEvent(E_BEGINFRAME, SAPPHIRE_HANDLER(WorkQueue, HandleBeginFrame));
	}
...
#else
		SAPPHIRE_LOGERROR("Can not create worker threads as threading is disabled");
#endif
	}
	...
	void WorkQueue::AddWorkItem(SharedPtr<WorkItem> item)
	{
		if (!item)
		{
			SAPPHIRE_LOGERROR("Null work item submitted to the work queue");
			return;
		}
	...
	void WorkQueue::HandleBeginFrame(StringHash eventType, VariantMap& eventData)
	{
		...
		{
			SAPPHIRE_PROFILE(CompleteWorkNonthreaded);
