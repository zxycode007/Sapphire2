...
namespace Sapphire
{
	...
	Variant Spline::GetPoint(float f) const
	{
		...
		default:
			SAPPHIRE_LOGERROR("Unsupported interpolation mode");
	...
	void Spline::SetKnot(const Variant& knot, unsigned index)
	{
		if (index < knots_.Size())
		{
			if (knots_.Size() > 0 && knots_[0].GetType() == knot.GetType())
				...
				SAPPHIRE_LOGERRORF("Attempted to set a Spline's Knot value of type %s where elements are already using %s",
	...
	void Spline::AddKnot(const Variant& knot)
	{
		if (knots_.Size() > 0 && knots_[0].GetType() == knot.GetType())
			...
			SAPPHIRE_LOGERRORF("Attempted to add Knot to Spline of type %s where elements are already using %s", knot.GetTypeName().CString(),
