...
namespace Sapphire
{
	...
	{
		Sapphire::Swap(length_, str.length_);
		Sapphire::Swap(capacity_, str.capacity_);
		Sapphire::Swap(buffer_, str.buffer_);
	}
...
#define GET_NEXT_CONTINUATION_BYTE(ptr) *ptr; if ((unsigned char)*ptr < 0x80 || (unsigned char)*ptr >= 0xc0) return '?'; else ++ptr;
	...
	String& String::AppendWithFormatArgs(const char* formatString, va_list args)
	{
		...
		while (true)
		{
			...
			default:
				//SAPPHIRE_LOGWARNINGF("Unsupported format specifier: '%c'", format);
				break;
			}
		}
	}
